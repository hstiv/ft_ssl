...
static void	padding(char *s, t_sha224 *data)
{
	int				i;
...
static void	fout(t_sha224 *data, int i)
{
	...
	while (j < BLOCK_64)
	{
		s0 = s0_224(data->w[j - 15]);
		s1 = s1_224(data->w[j - 2]);
...
static void	swap_words(t_sha224 *d)
{
	...
	while (++i < BLOCK_64)
	{
		ch = ch_224(d->ah[E], d->ah[F], d->ah[G]);
		t1 = d->ah[H] + sigma1_224(d->ah[E]) + ch + g_k[i] + d->w[i];
		t2 = sigma0_224(d->ah[A]) + ma_224(d->ah[A], d->ah[B], d->ah[C]);
		d->ah[H] = d->ah[G];
		d->ah[G] = d->ah[F];
		d->ah[F] = d->ah[E];
		d->ah[E] = d->ah[D] + t1;
